...
curve_layer <- stat_function(fun = dnorm, n = 101, args = list(mean = pop_mean, sd = pop_sd))
...
#                          samp_num, samp_mean, samp_sd)
  ...
  output$sample_count <- renderText(paste(sample_count_str, sprintf("%i", samp_num)))
  output$sample_mean <- renderText(paste(sample_mean_str, sprintf("%.2f", samp_mean)))
  output$standard_deviation <- renderText(paste(std_dev_str, sprintf("%.2f", samp_sd)))
  ...
  output$many_plot <- renderPlot(
     base_plot +
       geom_histogram(data = sample_means,
                      ...
                      color = "grey",
                      binwidth = bin_width) +
      stat_function(fun = function(x) dnorm(x, mean = pop_mean, sd = sd(samples)) * input$size_sample*input$number_samples * bin_width)
  )

  output$mean_of_means <- renderText(sprintf("Mean of %i sample means: %.2f", input$number_samples, mean(samples)))
})
